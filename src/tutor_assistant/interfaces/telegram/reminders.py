    ...
    try:
        async with async_session_factory() as session:
            async with session.begin():
                ...
                slots = await get_today_schedule(repo, chat_id, today)
...
async def _fetch_comment(chat_id: int, student_name: str) -> str:
    """Return formatted comment line for a student, or empty string if none."""
    try:
        async with async_session_factory() as session:
            async with session.begin():
                repo = SqlAlchemyStudentRepository(session)
                student = await repo.get_by_name(chat_id, student_name)
...
async def _maybe_send_preclass(
    context: ContextTypes.DEFAULT_TYPE,
    settings: ChatSettingsData,
    scheduled: ScheduledSlot,
    today: datetime.date,
) -> None:
    """Send a pre-class reminder for one slot if not already sent."""
    redis = _get_redis()
...
async def preclass_checker_callback(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for settings in all_settings:
        ...
        for scheduled in today_slots:
            ...
            if abs((scheduled.datetime_start - target_dt).total_seconds()) > 30:
                continue
            await _maybe_send_preclass(context, settings, scheduled, today)
